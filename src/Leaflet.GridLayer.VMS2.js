...
L.GridLayer.VMS2 = L.GridLayer.extend({
    ...
    initialize: function (options_) {
        if (!globalThis.vms2Context_) {
            ...
            let maxNumberOfWorkers_ = Math.max(navigator.hardwareConcurrency - 1, MIN_NUMBER_OF_WORKERS);

            for (let count_ = 0; count_ < maxNumberOfWorkers_; count_++) {
                let decodeWorker_ = new Worker(this._getWorkerURL_(new URL('decoder.js', import.meta.url)));

                decodeWorker_.onmessage = e => {
                    for (let tileData_ of e.data.tDs) {
                    ...
                    let resolveFunction_ = e.target.resolveFunction_;
    ...
    createTile: function (tileInfo_, doneFunction_) {
        ...
        for (let canvas_ of this.tileCanvases_) {
    ...
    getMapCanvas: async function (tileInfo_) {
        ...
        */
        if (this.options.accessKey) {
            ...
            let metaJson_ = JSON.stringify(meta_);
    ...
    _pruneTiles: function () {
        ...
        for (let key in this._tiles) {
            ...
            if (!tile.current) {
                if (coords.z - zoom > 2 / this.options.zoomStep || zoom - coords.z > 2 / this.options.zoomStep) {
                    ...
                    let latitudeMin = this._tileToLatitude_(coords.y + 1, coords.z, this.options.zoomPowerBase);
                    let longitudeMin = this._tileToLongitude_(coords.x, coords.z, this.options.zoomPowerBase);
                    let latitudeMax = this._tileToLatitude_(coords.y, coords.z, this.options.zoomPowerBase);
                    let longitudeMax = this._tileToLongitude_(coords.x + 1, coords.z, this.options.zoomPowerBase);
        ...
        for (let key1 in this._tiles) {
            let tile1 = this._tiles[key1];

            if (!tile1.current && tile1.reain) {
                for (let key2 in this._tiles) {
                    if (key2 === key1) {
                        continue;
                    }

                    let tile2 = this._tiles[key2];
    ...
    _checkAndSetDisplacement_: function (displacementLayers_, displacementLayerNames_, boxes_) {
        for (let box_ of boxes_) {
            if (box_.left_ > box_.right_) {
                let temp_ = box_.left_;
        ...
        for (let displacementLayerName_ of displacementLayerNames_) {
            let displacementLayer_ = displacementLayers_[displacementLayerName_];
            ...
            for (let box_ of boxes_) {
                ...
                let topLeftHash_ = (box_.left_ >> displacementLayer_.shift_) + 'x' + (box_.top_ >> displacementLayer_.shift_);
                let topRightHash_ = (box_.right_ >> displacementLayer_.shift_) + 'x' + (box_.top_ >> displacementLayer_.shift_);
                let bottomLeftHash_ = (box_.left_ >> displacementLayer_.shift_) + 'x' + (box_.bottom_ >> displacementLayer_.shift_);
                let bottomRightHash_ = (box_.right_ >> displacementLayer_.shift_) + 'x' + (box_.bottom_ >> displacementLayer_.shift_);

                if (displacementLayer_.regions_[topLeftHash_]) {
                    for (let hashedBox_ of displacementLayer_.regions_[topLeftHash_]) {
                ...
                if (displacementLayer_.regions_[topRightHash_] && topRightHash_ != topLeftHash_) {
                    for (let hashedBox_ of displacementLayer_.regions_[topRightHash_]) {
                        if (box_.left_ > hashedBox_.right_ ||
                            box_.right_ < hashedBox_.left_ ||
                            box_.bottom_ > hashedBox_.top_ ||
                            box_.top_ < hashedBox_.bottom_) { // Note: Top > Bottom!
                ...
                if (displacementLayer_.regions_[bottomLeftHash_] && bottomLeftHash_ != topLeftHash_ && bottomLeftHash_ != topRightHash_) {
                    for (let hashedBox_ of displacementLayer_.regions_[bottomLeftHash_]) {
                        if (box_.left_ > hashedBox_.right_ ||
                            box_.right_ < hashedBox_.left_ ||
                            box_.bottom_ > hashedBox_.top_ ||
                            box_.top_ < hashedBox_.bottom_) { // Note: Top > Bottom!
                ...
                if (displacementLayer_.regions_[bottomRightHash_] && bottomRightHash_ != topLeftHash_ && bottomRightHash_ != topRightHash_ && bottomRightHash_ != bottomLeftHash_) {
                    for (let hashedBox_ of displacementLayer_.regions_[bottomRightHash_]) {
                        if (box_.left_ > hashedBox_.right_ ||
                            box_.right_ < hashedBox_.left_ ||
                            box_.bottom_ > hashedBox_.top_ ||
                            box_.top_ < hashedBox_.bottom_) { // Note: Top > Bottom!
    ...
    _drawGeometry_: function (drawingInfo_, geometry_, dataOffset_ = 0) {
        let wkbType_ = geometry_.getUint32(dataOffset_, true);
        ...
        switch (wkbType_) {
            ...
            case 5: // WKBMultiLineString.
                {
                    ...
                    let numberOfLineStrings_ = geometry_.getUint32(dataOffset_, true);
                    dataOffset_ += 4;
            ...
            case 6: // WKBMultiPolygon.
                ...
                if (drawingInfo_.isIcon_ || drawingInfo_.isText_) {
                    ...
                    let numberOfPolygons_ = geometry_.getUint32(dataOffset_, true);
                    dataOffset_ += 4;
            ...
            case 7: // WKBGeometryCollection.
                {
                    ...
                    let numberOfGeometries_ = geometry_.getUint32(dataOffset_, true);
                    dataOffset_ += 4;
    ...
    _drawIcon_: function (drawingInfo_, x_, y_) {
        ...
        if (drawingInfo_.isText_ && drawingInfo_.text_) {
            ...
            let textWords_ = drawingInfo_.text_.replace(/-/g, '- ').split(' ');

            for (let textWord_ of textWords_) {
            ...
            for (let textLineInfo_ of textLineInfos_) {
            ...
            let textBoxHeight_ = drawingInfo_.fontSize_ * textLineInfos_.length;
            ...
            let spacingX_ = textBoxWidth_ * (drawingInfo_.displacementScaleX_ - 1);
            let spacingY_ = textBoxHeight_ * (drawingInfo_.displacementScaleY_ - 1);

            if (drawingInfo_.displacementScaleX_ > 0 && drawingInfo_.displacementScaleY_ > 0) {
                if (drawingInfo_.iconTextPlacement_ && drawingInfo_.isIcon_ && drawingInfo_.iconImage_) {
                    for (let placementCode_ in drawingInfo_.iconTextPlacement_) {
                        let gapX_ = drawingInfo_.iconWidth_ * drawingInfo_.iconTextPlacement_[placementCode_];
                        let gapY_ = drawingInfo_.iconHeight_ * drawingInfo_.iconTextPlacement_[placementCode_];
        ...
        if (textDisplacementBoxes_.length > 0) {
            for (let textDisplacementBox_ of textDisplacementBoxes_) {
                let textAndIconBoxes_ = [];
                ...
                if (this._checkAndSetDisplacement_(drawingInfo_.displacementLayers_, drawingInfo_.displacementLayerNames_, textAndIconBoxes_)) {
                    ...
                    if (drawingInfo_.isIcon_ && drawingInfo_.iconImage_) {
                        let iconX_ = drawingInfo_.iconImageOffsetX_ - drawingInfo_.iconWidth_ * drawingInfo_.iconMirrorX_ / 2;
                        let iconY_ = drawingInfo_.iconImageOffsetY_ - drawingInfo_.iconHeight_ * drawingInfo_.iconMirrorY_ / 2;
                    ...
                    if (drawingInfo_.isText_ && drawingInfo_.text_) {
                        if (!(textDisplacementBox_.left_ > drawingInfo_.mapArea_.right_ || textDisplacementBox_.right_ < drawingInfo_.mapArea_.left_ || textDisplacementBox_.top_ < drawingInfo_.mapArea_.bottom_ || textDisplacementBox_.bottom_ > drawingInfo_.mapArea_.top_)) { // Note: Top > Bottom!
                            ...
                            if (drawingInfo_.isIcon_ && drawingInfo_.iconPositions_[drawingInfo_.text_]) {
                                drawingInfo_.iconPositions_[drawingInfo_.text_].push({ x_: x_, y_: y_ });
                            }
                            ...
                            let textX_ = (x_ - drawingInfo_.drawingArea_.left_ + textDisplacementBox_.x_) * drawingInfo_.mapScale_;
    ...
    _drawLineString_(drawingInfo_, geometry_, dataOffset_) {
        ...
        let numberOfPoints_ = geometry_.getUint32(dataOffset_, true);
        dataOffset_ += 4;
        ...
        if (drawingInfo_.isIcon_ && drawingInfo_.iconImage_) { // Draw an icon and text on the line center.
            let halfLength_ = drawingInfo_.objectData_.length / 2;
            ...
            for (let pointIndex_ = 1; pointIndex_ < numberOfPoints_; pointIndex_++) {
                let x2_ = geometry_.getFloat32(dataOffset_, true);
                dataOffset_ += 4;

                let y2_ = geometry_.getFloat32(dataOffset_, true);
                dataOffset_ += 4;

                let deltaX_ = x2_ - x_;
                let deltaY_ = y2_ - y_;

                let segmentLength_ = Math.sqrt(deltaX_ * deltaX_ + deltaY_ * deltaY_);

                if (iconPositionLength_ + segmentLength_ > halfLength_) {
                    let factor_ = (halfLength_ - iconPositionLength_) / segmentLength_;
            ...
            if (drawingInfo_.iconPositions_[drawingInfo_.text_]) {
                for (let iconPosition_ of drawingInfo_.iconPositions_[drawingInfo_.text_]) {
                    let deltaX_ = x_ - iconPosition_.x_;
                    let deltaY_ = y_ - iconPosition_.y_;
            ...
            if (textWidth_ < drawingInfo_.objectData_.length) {
                ...
                for (let pointIndex_ = 0; pointIndex_ < numberOfPoints_; pointIndex_++) {
                    ...
                    points_.push({ x_: x_, y_: y_ });

                    if (pointIndex_ > 0) {
                        let deltaX_ = points_[pointIndex_ - 1].x_ - x_;
                        let deltaY_ = points_[pointIndex_ - 1].y_ - y_;
                ...
                if (textWidth_ < lineStringLength_) {
                    ...
                    for (let characterIndex_ = 0; characterIndex_ < text_.length; characterIndex_++) {
                        let characterWidth_ = globalThis.vms2Context_.fontCharacterWidths_[drawingInfo_.fontFamily_][drawingInfo_.fontStyle_][text_[characterIndex_]] * drawingInfo_.fontSize_ / 100;
                        ...
                        let factor_ = (characterOffset_ - lineOffset_) / segmentLengths_[pointsIndex_];
                        let textX_ = points_[pointsIndex_].x_ + (points_[pointsIndex_ + 1].x_ - points_[pointsIndex_].x_) * factor_;
                        let textY_ = points_[pointsIndex_].y_ + (points_[pointsIndex_ + 1].y_ - points_[pointsIndex_].y_) * factor_;
                    ...
                    if (drawingInfo_.displacementScaleX_ > 0 && drawingInfo_.displacementScaleY_ > 0) {
                        for (let characterInfo_ of characterInfos_) {
                            let left_ = characterInfo_.point_.x_ - drawingInfo_.fontSize_ * drawingInfo_.displacementScaleX_ / 2;
                            let right_ = characterInfo_.point_.x_ + drawingInfo_.fontSize_ * drawingInfo_.displacementScaleX_ / 2;
                            let top_ = characterInfo_.point_.y_ + drawingInfo_.fontSize_ * drawingInfo_.displacementScaleY_ / 2;
                            let bottom_ = characterInfo_.point_.y_ - drawingInfo_.fontSize_ * drawingInfo_.displacementScaleY_ / 2;
                    ...
                    if (this._checkAndSetDisplacement_(drawingInfo_.displacementLayers_, drawingInfo_.displacementLayerNames_, textBoxes_)) {
                        if (textIsVisible_) {
                            ...
                            for (let characterIndex_ = 0; characterIndex_ < text_.length; characterIndex_++) {
                                let angleStartPoint_ = characterIndex_ > 0 ? characterInfos_[characterIndex_ - 1].point_ : characterInfos_[0].point_;
                                let angleEndPoint_ = characterIndex_ < characterInfos_.length - 1 ? characterInfos_[characterIndex_ + 1].point_ : characterInfos_[characterIndex_].point_;
                                ...
                                let rotationAngleDelta_ = Math.abs(lastRotationAngle_ - characterRotationAngle_);
                            ...
                            if (maximumRotationAngleDelta_ < Math.PI * 2 / 4) {
                                ...
                                for (let characterIndex_ = 0; characterIndex_ < text_.length; characterIndex_++) {
                                    if (text_[characterIndex_] != ' ') {
                                        let matrix_ = new DOMMatrix().translate((characterInfos_[characterIndex_].point_.x_ - drawingInfo_.drawingArea_.left_) * drawingInfo_.mapScale_, (drawingInfo_.drawingArea_.top_ - characterInfos_[characterIndex_].point_.y_) * drawingInfo_.mapScale_).rotate(-characterInfos_[characterIndex_].rotationAngle_ * 180 / Math.PI);
    ...
    _drawPolygonsBoundary_: function (drawingInfo_, polygons_) {
        for (let polygonRings_ of polygons_) {
            for (let polygonPoints_ of polygonRings_) {
                let numberOfPoints_ = polygonPoints_.length;
                ...
                let deltaScale_ = Math.min(1, drawingInfo_.mapScale_);

                for (let pointIndex_ = 0; pointIndex_ < numberOfPoints_; pointIndex_++) {
                    let x_ = polygonPoints_[pointIndex_].x;
                    let y_ = polygonPoints_[pointIndex_].y;

                    let deltaLeft_ = Math.round((x_ - drawingInfo_.tileBoundingBox_.left) * deltaScale_);
                    let deltaRight_ = Math.round((x_ - drawingInfo_.tileBoundingBox_.right) * deltaScale_);
                    let deltaTop_ = Math.round((drawingInfo_.tileBoundingBox_.top - y_) * deltaScale_);
                    let deltaBottom_ = Math.round((drawingInfo_.tileBoundingBox_.bottom - y_) * deltaScale_);
    ...
    _convertGeojsonToTileLayer_: function (geojsonData_, tileLayer_, properties_) {
        switch (geojsonData_.type) {
            case 'FeatureCollection':
                for (let feature_ of geojsonData_.features) {
            ...
            case 'Point':
                {
                    ...
                    let x_ = this._longitudeToMeters_(geojsonData_.coordinates[0]);
                    let y_ = this._latitudeToMeters_(geojsonData_.coordinates[1]);
            ...
            case 'LineString':
                {
                    ...
                    for (let coordinate_ of geojsonData_.coordinates) {
                        let x_ = this._longitudeToMeters_(coordinate_[0]);
                        let y_ = this._latitudeToMeters_(coordinate_[1]);

                        if (length_ < 0) {
                            length_ = 0;
                        } else {
                            let deltaX_ = (x_ - previousX_);
                            let deltaY_ = (y_ - previousY_);
    ...
    _getTileLayers_: function (tileCanvas_, tileInfo_, mapStyle_) {
        return new Promise(resolve_ => {
            ...
            for (let layerName_ of mapStyle_.Order) {
                let layer_ = mapStyle_.Layers[layerName_];
                ...
                let layerLayout_ = layer_.LayoutLayers || [];
                ...
                if (Array.isArray(layerLayout_) && layerLayout_.length > 0) {
                    ...
                    for (let geometryType_ in layerLayout_) {
                        for (let osmKeyName_ in layerLayout_[geometryType_]) {
                            for (let osmValue_ of layerLayout_[geometryType_][osmKeyName_]) {
                ...
                layer_.needsAreaExtension_ = this._getLayerStyleType_(layer_) == 'text' || layer_.Grid || layer_.Save ? true : false;
                ...
                if (layer_.CustomData) {
                    ...
                    for (let layerLayoutId_ of layerLayoutIds_) {
                        if (!tileLayers_[layerName_]) {
                            tileLayers_[layerName_] = [];
                        }

                        let tileLayerData_ = { tileCanvas_, tileInfo_, dataLayerId_: layerLayoutId_, layerStyle_: layer_, tileIds_: [], objects: [], tileCount_: 0 };
    ...
    _drawSaveLayer_: async function (drawingInfo_, mapObjects_, tileInfo_, layer_) {
        ...
        for (let mapObject_ of mapObjects_) {
            if (!mapObject_) {
                continue;
            }
            ...
            let randomColor_ = (this.randomGenerator_.random_int() & 0xffffff);

            drawingInfo_.saveDataIds_[randomColor_] = drawingInfo_.objectData_;

            let red_ = (randomColor_ >> 16) & 0xff;
            let green_ = (randomColor_ >> 8) & 0xff;
            let blue_ = randomColor_ & 0xff;
    ...
    _drawBaseLayer_: async function (drawingInfo_, mapObjects_, tileInfo_, layer_) {
        ...
        if (!layer_.isGrid_ && layer_.Style && !layer_.Filters) {
            ...
            if (drawingInfo_.isStroked_) {
                if (objectStyle_.LineDash) {
                    ...
                    for (let dash_ of objectStyle_.LineDash) {
            ...
            if (objectStyle_.PatternFunction) {
                ...
                let patternName_ = objectStyle_.PatternFunction_(drawingInfo_.objectData_, tileInfo_.vms2TileZ_, this.randomGenerator_);

                if (patternName_) {
                    let pattern_ = await this._getPattern_(drawingInfo_.context_, patternName_);
        ...
        for (let mapObject_ of mapObjects_) {
            ...
            if (layer_.Filters) {
                ...
                let x_ = objectData_.Center.x;
                let y_ = objectData_.Center.y;
                ...
                if (drawingInfo_.isGrid_ && drawingInfo_.saveDataCanvas_) {
                    ...
                    let pixelX_ = Math.round((x_ - drawingInfo_.saveDataArea_.left_) * drawingInfo_.mapScale_);
                    let pixelY_ = Math.round((drawingInfo_.saveDataArea_.top_ - y_) * drawingInfo_.mapScale_);

                    if (pixelX_ >= 0 && pixelX_ < drawingInfo_.saveDataCanvas_.width && pixelY_ >= 0 && pixelY_ < drawingInfo_.saveDataCanvas_.height) {
                        let pixelIndex_ = (pixelX_ + pixelY_ * drawingInfo_.saveDataCanvas_.width) * 4;

                        let red_ = drawingInfo_.saveDataPixels_[pixelIndex_];
                        let green_ = drawingInfo_.saveDataPixels_[pixelIndex_ + 1];
                        let blue_ = drawingInfo_.saveDataPixels_[pixelIndex_ + 2];

                        let color_ = (red_ << 16) + (green_ << 8) + blue_;
                ...
                if (objectData_) {
                    for (let filter_ of layer_.Filters) {
            ...
            if (objectStyle_) {
                ...
                if (activeObjectStyle_.PatternFunction) {
                    let patternName_ = activeObjectStyle_.PatternFunction_(drawingInfo_.objectData_, tileInfo_.vms2TileZ_, this.randomGenerator_);
        ...
        for (let mapObject_ of mapObjects_) {
            ...
            if (objectStyle_) {
                ...
                if (activeObjectStyle_.IconFunction) {
                    let x_ = drawingInfo_.objectData_.Center.x;
                    let y_ = drawingInfo_.objectData_.Center.y;

                    this.randomGenerator_.init_seed((Math.round(x_) + 0xaffeaffe) * (Math.round(y_) + 0xaffeaffe));

                    let iconName_ = activeObjectStyle_.IconFunction_(drawingInfo_.objectData_, tileInfo_.vms2TileZ_, this.randomGenerator_);
    ...
    _getLayerStyleType_(layer_) {
        if (layer_.Style) {
            if (layer_.Style.IconFunction || layer_.Style.TextFunction) {
                ...
                for (let filter of layer_.Filters) {
    ...
    _requestStyle_: function () {
        return new Promise(resolve_ => {
            if (this.options.style.Order && Array.isArray(this.options.style.Order)) {
                ...
                let styleId_ = this.options.style;
                ...
                if (globalThis.vms2Context_.styleRequestQueues_[styleId_].length == 1) {
                    let url_ = new URL(this.options.styleUrl.replace('{style_id}', styleId_), window.location.origin);

                    let parameters_ = new URLSearchParams(url_.search);

                    let formBody_ = [];

                    for (let keyValuePair_ of parameters_.entries()) {
                    ...
                    fetch(url_.origin + url_.pathname, options_)
                        ...
                        .then(style_ => {
                            ...
                            for (let styleRequestResolve_ of globalThis.vms2Context_.styleRequestQueues_[styleId_]) {
                    ...
                    this._requestStyle_()
                        .then(style_ => {
                            ...
                            if (this.options.styleOverride) {
                                for (let key_ in style_) {
                                    if (style_.hasOwnProperty(key_)) {
                                        mapStyle_[key_] = style_[key_];
                                    }
                                }

                                for (let key_ in this.options.styleOverride) {
                            ...
                            if (!isNaN(tileInfo_.x) && !isNaN(tileInfo_.y) && !isNaN(tileInfo_.z)) {
                                ...
                                let degreesWidth_ = tileInfo_.mapBounds_.longitudeMax_ - tileInfo_.mapBounds_.longitudeMin_;

                                let normalizedWidth_ = degreesWidth_ / 360;
                                let normalizedHeight_ = this._latitudeToNormalized_(tileInfo_.mapBounds_.latitudeMin_) - this._latitudeToNormalized_(tileInfo_.mapBounds_.latitudeMax_);

                                let normalizedRatio_ = normalizedWidth_ / normalizedHeight_;
                                let mapRatio_ = tileInfo_.width_ / tileInfo_.height_;
                                ...
                                let tileSize_ = this.tileSize_ * tileInfo_.dpi / DEFAULT_DISPLAY_DPI_;
                            ...
                            this._getTileLayers_(tileCanvas_, tileInfo_, mapStyle_).then(async tileLayers_ => {
                                ...
                                let drawingInfo_ = {
                                    mapArea_: mapArea_,
                                    extendedMapArea_: extendedMapArea_,
                                ...
                                if (this.options.displacementIcons) {
                                    let displacementBoxes_ = [];

                                    for (let displacementIcon_ of this.options.displacementIcons) {
                                        let width_ = displacementIcon_.size[0];
                                        let height_ = displacementIcon_.size[1];

                                        let anchorX_ = displacementIcon_.anchor ? displacementIcon_.anchor[0] : (width_ / 2);
                                        let anchorY_ = height_ - (displacementIcon_.anchor ? displacementIcon_.anchor[1] : (height_ / 2));

                                        let left_ = this._longitudeToMeters_(displacementIcon_.longitude) - anchorX_ * tileInfo_.width_ / (this.tileSize_ * drawingInfo_.mapScale_);
                                        let right_ = this._longitudeToMeters_(displacementIcon_.longitude) + (width_ - anchorX_) * tileInfo_.width_ / (this.tileSize_ * drawingInfo_.mapScale_);
                                        let top_ = this._latitudeToMeters_(displacementIcon_.latitude) + (height_ - anchorY_) * tileInfo_.width_ / (this.tileSize_ * drawingInfo_.mapScale_);
                                        let bottom_ = this._latitudeToMeters_(displacementIcon_.latitude) - anchorY_ * tileInfo_.width_ / (this.tileSize_ * drawingInfo_.mapScale_);
                                ...
                                for (let layerName_ of mapStyle_.Order) {
                                    ...
                                    let mapObjects_ = tileLayers_[layerName_] || [];
                                    ...
                                    if (layer_.Grid) {
                                        ...
                                        let gridZoomScale_ = 1 / drawingInfo_.userMapScale_ / Math.pow(DEFAULT_DISPLAY_DPI_ * drawingInfo_.mapScale_ / drawingInfo_.userMapScale_ / tileInfo_.dpi_, layer_.Grid.ZoomScale || 1);

                                        let gridSize_ = [layer_.Grid.Size[0] * drawingInfo_.objectScale_ * gridZoomScale_, layer_.Grid.Size[1] * drawingInfo_.objectScale_ * gridZoomScale_];
                                        ...
                                        let worldTop_ = this._tileYToMeters_(0, 0);
                                        let worldLeft_ = this._tileXToMeters_(0, 0);

                                        let gridStartIndexX_ = Math.floor((drawingInfo_.saveDataArea_.left_ - worldLeft_) / gridSize_[0]) - 1;
                                        let gridIndexY_ = Math.floor((worldTop_ - drawingInfo_.saveDataArea_.top_) / gridSize_[1]) - 1;

                                        let gridLeft_ = gridStartIndexX_ * gridSize_[0] + worldLeft_;
                                        let gridRight_ = drawingInfo_.saveDataArea_.right_;
                                        let gridTop_ = worldTop_ - gridIndexY_ * gridSize_[1];
                                        let gridBottom_ = drawingInfo_.saveDataArea_.bottom_;
                                        ...
                                        for (let gridY_ = gridTop_; gridY_ >= gridBottom_; gridIndexY_++) {
                                            ...
                                            let gridSkewX_ = (gridIndexY_ * gridSkew_[0]) % gridSize_[0];
                                            ...
                                            for (let gridX_ = gridLeft_, gridIndexX_ = gridStartIndexX_; gridX_ <= gridRight_; gridIndexX_++) {
                                                ...
                                                let gridSkewY_ = (gridIndexX_ * gridSkew_[1]) % gridSize_[1];
                                        ...
                                        for (let gridPoint_ of gridPoints_) {
                                            let center_ = {};
                                            ...
                                            let objectInfo_ = { Center: center_, Envelope: envelope_, Angle: gridPoint_.angle_ };
                                    ...
                                    if (layer_.SortFunction) {
                                        let sortFunction_ = new Function('a', 'b', 'return (' + layer_.SortFunction + ')');
                                    ...
                                    let layerCanvasNames_ = ['']; //layer_.CanvasNames || [ '' ];

                                    if (layer_.Save) {
                                        layerCanvasNames_.push('save');
                                    }

                                    for (let layerCanvasName_ of layerCanvasNames_) {
                                        if (layerCanvasName_ == 'save') {
                                            if (!drawingInfo_.saveDataCanvas_) {
                                                let saveDataCanvas_ = null;

                                                for (let canvas_ of this.saveDataCanvases_) {
                                                    if (!canvas_.inUse) {
                                ...
                                if (this.options.type != 'text') {
                                    if (mapStyle_.BackgroundPatternFunction) {
                                        ...
                                        let patternName_ = mapStyle_.BackgroundPatternFunction_(null, tileInfo_.vms2TileZ_, this.randomGenerator_);
    ...
    _getCachedTile_: function (layerId_, x_, y_, z_, tileLayer_) {
        ...
        let ids_ = layerId_.split('|');

        if (!(ids_.length == 1 || ids_[2] != 'Points')) {
            detailZoom_ = 14;
        }

        let tileWeight_ = Math.pow(4, 16 - z_);
        let matchingTilesWeight_ = 0;

        let layerMap_ = globalThis.vms2Context_.tileCacheLayerMaps_[layerId_];

        if (layerMap_) {
            for (let keyValuePair_ of layerMap_) {
                if (keyValuePair_[1].detailZoom_ != detailZoom_) {
                    continue;
                }

                let deltaZ_ = keyValuePair_[1].z_ - z_;
    ...
    _processTileLayerRequests_: async function (tileLayerRequestInfo_) {
        if (!tileLayerRequestInfo_.requestInProcess_) {
            ...
            while (tileLayerRequestInfo_.tileInfos_.length > 0) {
                let tileInfo_ = tileLayerRequestInfo_.tileInfos_.shift();

                let dataLayerId_ = tileInfo_.tileLayerData_.dataLayerId_;

                let x_ = tileInfo_.x;
                let y_ = tileInfo_.y;
                let z_ = Math.floor(tileInfo_.z);

                let tileLayerData_ = tileInfo_.tileLayerData_;
    ...
    _getTileLayer_: function (tileLayerData_) {
        return new Promise((resolve_, reject_) => {
            ...
            let tileLayerRequestInfo_ = globalThis.vms2Context_.tileLayerRequestInfos_[tileLayerData_.dataLayerId_];
            ...
            for (let fetchTileY_ = fetchTileStartY_; fetchTileY_ <= fetchTileEndY_; fetchTileY_++) {
                for (let fetchTileX_ = fetchTileStartX_; fetchTileX_ <= fetchTileEndX; fetchTileX_++) {
                    tileLayerRequestInfo_.tileInfos_.push({ x: fetchTileX_, y: fetchTileY_, z: fetchTileZ_, tileLayerData_: tileLayerData_ });
    ...
    _preparePolygon_: function (drawingInfo_, geometry_, dataOffset_, polygons_) {
        ...
        let numberOfRings_ = geometry_.getUint32(dataOffset_, true);
        dataOffset_ += 4;
    ...
    _requestFontFace_: function (style_) {
        return new Promise(resolve => {
            ...
            let font_ = new FontFace(style_.FontFamily, 'url(\'' + this.options.assetsUrl + '/fonts/' + fontName_ + '\')', { style: fontStyle_, weight: fontWeight_ });
            ...
            font_.load().then(() => {
                ...
                for (let resolveFunction_ of globalThis.vms2Context_.fontFaceCache_[fontName_].resolveFunctions_) {
                    if (resolveFunction_) {
                        resolveFunction_();
                    }
                }
    ...
    _requestImage_: function (imageUrlString_) {
        return new Promise((resolve_, reject_) => {
            let imageCache_ = globalThis.vms2Context_.imageCache_;
            ...
            let imageUrl_ = new URL(imageUrlString_, window.location.origin);
                ...
                fetch(imageUrlString_)
                    ...
                    .then(svgImage_ => {
                        image_.onload = () => {
                            ...
                            for (let resolveFunction_ of imageCache_[imageUrlString_].resolveFunctions_) {
                                if (resolveFunction_) {
            ...
            imageCache_[imageUrlString_] = { isLoading_: true, resolveFunctions_: [resolve_], image_: image_ };
        });
    },
    _setVoidTileArea_(x_, y_, z_) {
        let tileLeft_ = x_ << (16 - z_);
        let tileRight_ = (x_ + 1) << (16 - z_);
        let tileTop_ = y_ << (16 - z_);
        let tileBottom_ = (y_ + 1) << (16 - z_);

        let voidTileAreas_ = [];

        voidTileAreas_.push({ tileLeft_, tileRight_, tileTop_, tileBottom_ });

        for (let voidTileArea_ of this.voidTileAreas_) {
    ...
    _requestTileDbInfos_: function () {
        return new Promise(resolve_ => {
            ...
            if (this.tileDbInfos_) {
                ...
                let resolves_ = this.tileDbInfosResolves_;

                resolves_.push(resolve_);

                if (resolves_.length == 1) {
                    let tileDbInfosUrlParts_ = this.options.tileUrl.split('?');
    ...
    _requestTile_: function (dataLayerId_, x_, y_, z_, tileLayerData_) {
        return new Promise(resolve_ => {
            ...
            let tileLatitudeMin_ = this._tileToLatitude_(y_ + 1, z_);
            let tileLatitudeMax_ = this._tileToLatitude_(y_, z_);
            let tileLongitudeMin_ = this._tileToLongitude_(x_, z_);
            let tileLongitudeMax_ = this._tileToLongitude_(x_ + 1, z_);

            for (let tileDbInfo_ of this.tileDbInfos_) {
                if (tileDbInfo_.infos.length > 0) {
                    let boundingBox = tileDbInfo_.infos[0].bounding_box;
            ...
            fetch(new URL(tileUrl_, window.location.origin), { signal: tileLayerData_.tileCanvas_.abortController_.signal })
                ...
                .then(rawData_ => {
                    ...
                    let decodeData_ = { lId: dataLayerId_, datas: [] };

                    let rawDataDataView_ = new DataView(rawData_);
                    ...
                    while (tileCount_ > 0) {
                        let tileX_ = rawDataDataView_.getUint32(rawDataOffset_, true);
                        rawDataOffset_ += 4;

                        let tileY_ = rawDataDataView_.getUint32(rawDataOffset_, true);
                        rawDataOffset_ += 4;

                        let tileZ_ = rawDataDataView_.getUint32(rawDataOffset_, true);
                        rawDataOffset_ += 4;

                        let detailZoom_ = rawDataDataView_.getUint32(rawDataOffset_, true);
                        rawDataOffset_ += 4;

                        let dataSize_ = rawDataDataView_.getUint32(rawDataOffset_, true);
                        rawDataOffset_ += 4;
                    ...
                    let decodeFunction_ = () => {
                        for (let decodeWorker_ of globalThis.vms2Context_.decodeWorkers_) {
                            if (!decodeWorker_.resolveFunction_) {
                                let decodeEntry_ = globalThis.vms2Context_.decodeQueue_.shift();
    ...
    async _getPattern_(context_, patternName_) {
        if (!globalThis.vms2Context_.patternCache_[patternName_]) {
            ...
            let patternImage_ = await this._requestImage_(patternUrl_);
